...
**Port:** 4009  
**Description:** Workflow orchestrator: state machine (XState), domain events, CQRS, Zod validation
...
- [ ] Shadow mode via gateway flag `ROUTE_PERMOHONAN=new`
- [ ] Cutover when shadow zero-mismatch 1 week

## Run

```bash
pnpm --filter @sipera/permohonan run dev
curl http://localhost:4009/_health
```
